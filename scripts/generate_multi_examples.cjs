...
function buildExamplesForWord(item) {
  ...
  const cleanHanzi = orig.hanzi.replace(/[。？！?!]$/, '');
  const cleanPinyin = orig.pinyin.replace(/[\.\?!]$/, '');

  const ex1 = {
    hanzi: cleanHanzi + '。',
    pinyin: cleanPinyin + '.',
    thaiMeaning: `${orig.thaiMeaning} (บริบท: การใช้พื้นฐาน)`
  };

  const ex2 = {
    hanzi: `你知道这个${h}怎么用吗？`,
    pinyin: `Nǐ zhīdào zhège ${p} zěnme yòng ma?`,
    thaiMeaning: `คุณรู้ไหมว่า${m}นี้ใช้ยังไง? (บริบท: ประโยคคำถาม/สนทนา)`
  };

  const ex3 = {
    hanzi: `我们经常在生活中用到${h}。`,
    pinyin: `Wǒmen jīngcháng zài shēnghuó zhōng yòng dào ${p}.`,
    thaiMeaning: `พวกเรามักจะใช้${m}ในชีวิตประจำวันบ่อยๆ (บริบท: บริบทชีวิตประจำวัน)`
  };
...
console.log('Successfully updated vocabularyData.js with 2-3 multi-context examples per word!');
